...
X_price = location_science.copy()
X_price.price_Close = X_price.price_Close.shift(-1)
X = X_price[[col for col in location_science.columns.tolist() if col!='price_Close']].sort_index()[location_science.index <'2019-03-31']
y_price = X_price['price_Close'].dropna()
...
X_array, y_array = X.values, y_price.values
tscv = TimeSeriesSplit(n_splits=5)

## TODO: 1 day forward price prediction
for i, (train_index, test_index) in enumerate(tscv.split(X_array)):
    ...
    mse_1 = mean_squared_error(scaled_y_test, predict_y_test)


    model2 = best_RandForestReg(scaled_X_train, scaled_y_train)
    predict_y_test_2 = model2.predict(scaled_X_test)
    mse_2 = mean_squared_error(scaled_y_test, predict_y_test_2)


    plt.plot(y_test_scaler.inverse_transform(predict_y_test), color='b', label='LASSO MSE: %s' %mse_1.round(3))
    plt.plot(y_test_scaler.inverse_transform(predict_y_test_2), color='g', label='Random Forest Regressor MSE: %s' %mse_2.round(3))
    plt.plot(y_test_scaler.inverse_transform(scaled_y_test), color='r')
    plt.title("LASSO and Random Forest 1 day Forward Pricing model")
    plt.legend()
    plt.savefig("LASSO and Random Forest 1 day Forward Pricing model CV %s"%i + ".png")
    # plt.show()
    plt.close()
...
X_scaler, y_scaler = StandardScaler().fit(X), StandardScaler().fit(y_price.values.reshape(-1, 1))
scaled_X, scaled_y = X_scaler.transform(X), y_scaler.transform(y_price.values.reshape(-1, 1))
...
print("Best MSE using built-in LassoCV: ", model.mse_path_)
